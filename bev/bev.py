...
# Required nuScenes modules:
# --------------------------------
from nuscenes.map_expansion.map_api import NuScenesMap
from nuscenes.map_expansion.bitmap import BitMap


# Class for nuScenes BEV generation:
# -----------------------------
class NuScenesBEV:
    """
    ...
    The colour coding is done in the HSV space using Hue as the dimension to code
    object class, saturation is used to code confidence. NuScenes BEVs are ground-truth and 
    hence, the confidence will be 100% and hence, saturation=1. The Value space can be used 
    to code velocity but, at present is not considered and kept at 1.
    """

    def __init__(self, nuscenes_data_path, out_path, data_version, distance, fig_size, dpi):
        """
        :param nuscenes_data_path: The path to the directory where nuScenes meta-data is stored

        :param out_path: The path to the directory where the output BEVs will be stored

        :param data_version: The version of the nuScenes dataset that is downloaded

        :param distance: The distance that must be covered by the BEV image from the center

        :param fig_size: The desired BEV image output size

        :param dpi: Dots per inch of the output BEV image
        """
    ...
    def plot_bev(self, plotting_data, location, bev_save_path):
        ...
        :param plotting_data: .csv files containing the necessary information

        :param location: Location info of the BEV scene

        :param bev_save_path: The path to the directory where the BEV images will be stored
        """
            ...
            with open(Path(__code_file)) as __code_file:
                ...
                for i in range(0, len(plotting_data)):
                    if plotting_data['category'][i] in __code:
                        ...
                        """
                        Here, patch refers to rectangles representing the bounding boxes.
                        """
                ...
                """
                Crop the area as desired. Currently the 'distance' is cropped from the center in
                all four directions.
                """
                ...
                plt.savefig(f'{bev_save_path}.png', bbox_inches='tight', dpi=self.__dpi, pad_inches=0)
                plt.close()
...
class ObjTrackBEV:
    ...
    colour coding is done to match with the ground-truth nuScenes BEVs.
    """
    ...
    def plot_bev(self, plotting_data, location, bev_save_path):
        """
        The method plots the BEVs. It reads the map information provided by
        nuScenes and plots the objects on the map and also crops the map based on
        user-definition. The BEV images are saved scene-wise in separate folders.
